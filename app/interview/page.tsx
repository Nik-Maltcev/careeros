    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-emerald-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg font-medium text-foreground">Подготавливаем интервью...</p>
          <p className="text-muted-foreground text-sm mt-2">{debugInfo}</p>
          <div className="mt-4 text-xs text-muted-foreground/80 max-w-md">
            <p>Specialty: {specialty}</p>
            <p>Level: {level}</p>
            <p>Используется модель Sonar Pro от Perplexity AI</p>
            <p className="text-primary mt-2">✅ API ключи настроены автоматически</p>
          </div>
        </div>
      </div>
    )
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-emerald-50 flex items-center justify-center p-4">
        <Card className="border-destructive/30 bg-white/90 backdrop-blur-sm shadow-xl max-w-md w-full">
          <CardContent className="p-6 text-center">
            <AlertCircle className="w-12 h-12 text-destructive mx-auto mb-4" />
            <h2 className="text-xl font-semibold text-foreground mb-2">Ошибка</h2>
            ...
            <div className="flex gap-2">
              <Button
                ...
                variant="outline"
                className="rounded-full border-border/60 bg-white text-foreground hover:bg-white"
              >
                Попробовать снова
              </Button>
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-foreground">AI Interview</h1>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              <Badge
                className={`text-xs ${apiStatus.perplexity ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-rose-50 text-destructive border-rose-200"}`}
              >
                {apiStatus.perplexity ? <Wifi className="w-3 h-3 mr-1" /> : <WifiOff className="w-3 h-3 mr-1" />}
                Perplexity
              </Badge>
              <Badge
                className={`text-xs ${apiStatus.openai ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-amber-50 text-amber-600 border-amber-200"}`}
              >
          ...
          <Card className="bg-amber-50 border-amber-200 mb-6">
            <CardContent className="p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-amber-600 font-medium text-sm">Демо режим</h3>
        ...
        {/* Question Card */}
        <Card className="bg-white/90 border-border/60 backdrop-blur-sm rounded-3xl shadow-lg mb-6">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg font-medium text-foreground md:text-xl">{currentQuestion?.text}</CardTitle>
        ...
        {/* Mobile Tips */}
        {isMobile && (
          <Card className="bg-sky-50 border-sky-200 mb-6">
            <CardContent className="p-4">
              <div className="flex items-start space-x-3">
                <Smartphone className="w-5 h-5 text-sky-500 flex-shrink-0 mt-0.5" />
                <div>
                  <h3 className="text-sky-500 font-medium text-sm mb-1">Советы для мобильных устройств:</h3>
                  <ul className="text-sky-600 text-xs space-y-1">
          ...
          <Card className="bg-emerald-50 border-emerald-200 mb-6">
            <CardContent className="p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-emerald-500 font-medium text-sm">Ответ записан!</h3>
